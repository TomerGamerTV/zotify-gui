...
def download_episode(episode_id, pbar_stack: list | None = None) -> None:
    ...
    if podcast_name is None or episode_name is None or duration_ms is None:
        ...
        with Loader(PrintChannel.PROGRESS_INFO, "Preparing download..."): 
            ...
            # i think all podcasts are in .mp3 format... better fix using ffprobe coming soon
            filename = f"{podcast_name} - {episode_name}.mp3"
            episode_path = PurePath(Zotify.CONFIG.get_root_podcast_path()) / podcast_name / filename
